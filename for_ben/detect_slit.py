...
directory = "/home/laura/Projects/microcalcs/output/ccd_trs/"
filenames = []
count=0
valid_count=0
...
for file in sorted(os.listdir(directory), key=numericalSort):
    if file.endswith(".nc"):
        ...
        for i in range(len(new_vals)):
        	for j in range(len(new_vals)):
        		if -3.0 <= (int(i)-centre) <= 3.0:
        			print("vals: ", i, int(i)-centre)
        			if -0.5 <= (int(j)-centre) <= 0.5:
        				total_count += new_vals[i][j]
        				#print("valid value: ", new_vals[i][j])
        				valid_count+=1
        	
        g = open('/home/laura/Projects/microcalcs/output/for_ben/Raman_weight_rectangle.txt', 'a')
        g.write("{}\n".format(total_count))
